...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const metadata = await ethers.getContract('MetadataService', owner)
  ...
  if (network.name !== 'hardhat' && network.name !== 'localhost') {
    ...
    try {
      ...
      console.log("NameWrapper Contract verified successfully!");
    } catch (err) {
      console.log("NameWrapper Verification failed:", err);
    }
  ...
  if (resolver === ethers.constants.AddressZero) {
    console.log(
      `No resolver set for .web3; not setting interface ${interfaceId} for NameWrapper`,
    )
    return
  }
  ...
  console.log(
    `Setting NameWrapper interface ID ${interfaceId} on .web3 resolver (tx: ${tx3.hash})...`,
  )
  await tx3.wait()
}
